...
class RoomBookingAssistant {
  ...
  private analyzeBookingInput(userInput: string): any {
    ...
    const criticalFields = ['roomName', 'topic', 'date', 'time'];
    const criticalExtracted = criticalFields.filter(field => {
      const value = extracted[field as keyof Booking];
      return value !== null && value !== undefined && value !== '' && value !== 0;
    });
    
    if (criticalExtracted.length >= 4) {
      // If we have 4+ critical fields, consider it high confidence
    ...
    const finalConfidence = criticalExtracted.length >= 4 ? Math.max(confidence, 0.8) : Math.max(confidence, 0.7);
...
- Data lengkap: ${confidence > 0.8 ? 'YA' : 'TIDAK'}
